...
void core400sComponent::process_packet()  {
        ...
        switch (packet_type) {
            case 0x01B040:  //Status packet
            case 0x01B140:
                if (rx_buf.size() != 28)
                    return;

                sprintf(buf, "%d.%d.%d",rx_buf[12], rx_buf[11],rx_buf[10]);
                defer("fw", [=, this]()-> void {  textsensor_mcu_fw->publish_state(buf);    });

                switch(rx_buf[13]) {  //Power
                    case 0x00:
                    defer("def_pw", [=, this]()-> void {  textsensor_power->publish_state("Off");    });
                    break;

                    case 0x01:
                    defer("def_pw", [=, this]()-> void {  textsensor_power->publish_state("On");    });
                    break;

                    default:
                    sprintf(buf, "Error %X", rx_buf[13]);
                    defer("def_pw", [=, this]()-> void {  textsensor_power->publish_state(buf);    });
                }

                switch(rx_buf[14]) {  //Fan Mode
                    case 0x00:
                    defer("def_fm", [=, this]()-> void {  textsensor_fan_mode->publish_state("Manual");    });
                    break;

                    case 0x01:
                    defer("def_fm", [=, this]()-> void {  textsensor_fan_mode->publish_state("Sleep");    });
                    break;

                    case 0x02:
                    defer("def_fm", [=, this]()-> void {  textsensor_fan_mode->publish_state("Auto");    });
                    break;                

                    default:
                    sprintf(buf, "Error %X", rx_buf[14]);
                    defer("def_fm", [=, this]()-> void {  textsensor_fan_mode->publish_state(buf);    });
                }   

                switch(rx_buf[15]) {  //Fan Speed
                    case 0x00:
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state("Sleep");    });
                    break;

                    case 0x01:
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state("Low");    });
                    break;

                    case 0x02:
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state("Medium");    });
                    break;  

                    case 0x03:
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state("High");    });
                    break;
                    
                    case 0x04:
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state("Turbo");    });
                    break;                    

                    case 0xFF:
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state("Off");    });
                    break;  

                    default:
                    sprintf(buf, "Error %X", rx_buf[18]);
                    defer("def_fs", [=, this]()-> void {  textsensor_fan_speed->publish_state(buf);    });
                } 

                switch(rx_buf[17]) {  //Screen
                    case 0x00:
                    defer("def_sc", [=, this]()-> void {  textsensor_display_lit->publish_state("Off");    });
                    break;

                    case 0x64:
                    defer("def_sc", [=, this]()-> void {  textsensor_display_lit->publish_state("On");    });
                    break;

                    default:
                    sprintf(buf, "Error %X", rx_buf[16]);
                    defer("def_sc", [=, this]()-> void {  textsensor_display_lit->publish_state(buf);    });
                }
                                        
                if(rx_buf[22]==0xFF && rx_buf[21]==0xFF)  //0xFFFF returned when off, samples every 15 minutes while off.
                    defer("def_pm", [=, this]()-> void {  sensor_pm25->publish_state(NAN);       });             
                else
                    defer("def_pm", [=, this]()-> void {  sensor_pm25->publish_state( (float) ((rx_buf[22]<<8) + rx_buf[21]) / 1.0);    });

                switch(rx_buf[23]) {  //Display Lock
                    case 0x00:
                    defer("def_dl", [=, this]()-> void {  textsensor_display_locked->publish_state("Unlocked");    });
                    break;

                    case 0x01:
                    defer("def_dl", [=, this]()-> void {  textsensor_display_locked->publish_state("Locked");    });
                    break;

                    default:
                    sprintf(buf, "Error %X", rx_buf[23]);
                    defer("def_dl", [=, this]()-> void {  textsensor_display_locked->publish_state(buf);    });
                }

                switch(rx_buf[24]) {  //Fan Mode
                    case 0x00:
                    defer("def_fa", [=, this]()-> void {  textsensor_fan_auto_mode->publish_state("Default");    });
                    break;

                    case 0x01:
                    defer("def_fa", [=, this]()-> void {  textsensor_fan_auto_mode->publish_state("Quiet");    });
                    break;

                    case 0x02:
                    defer("def_fa", [=, this]()-> void {  textsensor_fan_auto_mode->publish_state("Room Size");    });
                    break;                

                    default:
                    sprintf(buf, "Error %X", rx_buf[24]);
                    defer("def_fa", [=, this]()-> void {  textsensor_fan_auto_mode->publish_state(buf);    });
                }

                areaL=rx_buf[25];
                areaH=rx_buf[26];
                defer("def_ar", [=, this]()-> void {  sensor_room_size->publish_state( (rx_buf[26]<<8) + rx_buf[25]);    });
...
void core400sComponent::set_fan_manual_turbo(void) {
    ESP_LOGD(TAG, "set_fan_manual_turbo");
    send_command(cmd_set_fan_manual_turbo);
}
